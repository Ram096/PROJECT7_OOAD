...
public class ExpertCook implements CookMethod, SysOut{
    public void cook(Pizza pizza, Customer c, Inventory i) {
        ...
        if (rand1 < 0.1) {
            ...
            pizza.setTip(tip/100.0);
            if (tip < 10) {
                out("The pizza was over cooked by our expert chef and the person gave a "+tip+"% tip being "+Utility.asDollar(pizza.getPrice(c.prefTopping, c.prefSize, pizza.getCookCond()) * (tip/100.0))+" to the user");
            } else if (tip < 15) {
                out("The pizza was over cooked by our expert chef and the person gave a "+tip+"% tip being "+Utility.asDollar(pizza.getPrice(c.prefTopping, c.prefSize, pizza.getCookCond()) * (tip/100.0))+" to the user");
            } else {
                out("The pizza was over cooked by our expert chef and the person gave a "+tip+"% tip being "+Utility.asDollar(pizza.getPrice(c.prefTopping, c.prefSize, pizza.getCookCond()) * (tip/100.0))+" to the user");
