...
public class BadCook implements CookMethod, SysOut{
    public void cook(Pizza pizza, Customer c, Inventory i) {
        ...
        if (rand1 < 0.3) {
            ...
            pizza.setTip(tip/100.0);
            if (tip < 10) {
                out("The pizza was over cooked by our new chef and the person was not happy and gave a bad tip being a "+tip+"% tip being "+Utility.asDollar(pizza.getPrice(c.prefTopping, c.prefSize, pizza.getCookCond()) * (tip/100.0))+" to the user");
            } else if (tip < 15) {
                out("The pizza was over cooked by our new chef and the person felt generous by giving a "+tip+"% tip being "+Utility.asDollar(pizza.getPrice(c.prefTopping, c.prefSize, pizza.getCookCond()) * (tip/100.0))+" to the user");
            } else {
                out("The pizza was over cooked by our new chef and the person luckily gave a nice tip being a "+tip+"% tip being "+Utility.asDollar(pizza.getPrice(c.prefTopping, c.prefSize, pizza.getCookCond()) * (tip/100.0))+" to the user");
            }
        }
